...
export default function HistoricoRow({dados}: {dados: RequiProps})
{
    return (
        <div className={`${styles.card} ${styles[dados.area]}`}>
            ...
            <div>
                <p className={`${styles.badge} ${dados.status === null ? styles.pendente : dados.status ? styles.aprovado : styles.rejeitado}`}>{dados.status === null ? "Pendente" : dados.status ? "Aprovado" : "Reprovado"}</p>
            </div>
        </div>
    );
}