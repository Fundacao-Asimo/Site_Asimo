...
export default function ConteudoFreeTimeGeral({listFree, listMembros}: {listFree: FreeTimeProps[], listMembros: MembroProps[]})
{
    ...
    useEffect(() => {
        if (listFree.length === 0 || listMembros.length === 0) {
        ...
        setSelectedMembers(listMembros.map((m) => m.id));
    }, [listFree, listMembros, router]);
                                    ...
                                    {dias.map(d => {
                                        return (
                                            <td
                                                key={d}
                                                onClick={() => {
                                                    setMembrosModel(listMembros.filter((m: MembroProps) => data[d][i].includes(m.id)));
